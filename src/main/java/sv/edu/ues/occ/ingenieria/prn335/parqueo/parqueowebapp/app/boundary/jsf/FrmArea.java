...
public class FrmArea extends AbstractFrm<Area> implements Serializable {
    ...
    public void inicializar() {
        ...
        if (lista != null && !lista.isEmpty()) {
            for (Area next : lista) {
                if (next.getIdAreaPadre() == null) {// es raiz, se itera a partir de el
    ...
    public Area getObjetoPorId(String id) {
        if (id != null && this.modelo != null && this.modelo.getWrappedData() != null) {
            return modelo.getWrappedData().stream().
                    filter(r -> r.getIdArea().toString().equals(id)).collect(Collectors.toList()).get(0);
        }
        return null;
    }
    ...
    public List<Area> cargarDatos(int primero, int tamanio) {
        ...
        List<Area> lista = aBean.findByIdPadre(idPadre, 0, 1000000);
        return lista;
    }
